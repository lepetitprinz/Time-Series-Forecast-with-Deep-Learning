...
class Models(object):
    def __init__(self, window: WindowGenerator):
        ...
        self.n_hidden = config.N_HIDDEN    # Seq to Seq Model Units
    ...
    def train(self, model: Model, save_nm: str):
        ...
        # train
        history = model.fit(self.window.train, epochs=config.EPOCHS,
    ...
    def predict(self, model: Model, data: pd.DataFrame, normalize_stats: tuple):
        ...
        yhat = yhat * std[config.COL_TARGET] + mean[config.COL_TARGET]
